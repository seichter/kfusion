...
inline __host__ __device__ uint min(uint2 a) { return min(a.x, a.y); }
inline __host__ __device__ uint min(uint3 a) { return min(a.x, min(a.y, a.z)); }
inline __host__ __device__ uint min(uint4 a) {
  return min(min(a.x, a.y), min(a.z, a.w));
}

inline __host__ __device__ float min(float2 a) { return fminf(a.x, a.y); }
...
inline __host__ __device__ uint max(uint3 a) { return max(a.x, max(a.y, a.z)); }
