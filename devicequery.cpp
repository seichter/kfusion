...
void runtimeTest() {
  ...
  float *vecData = (float *)malloc(sizeof(float) * dataSize);

  cudaError_t err = cudaMalloc((void **)&vecData, sizeof(float) * dataSize);
...
*/
  ...
  for (dev = 0; dev < deviceCount; ++dev) {
    ...
    shrLog("  Max Texture Dimension Size (x,y,z)             1D=(%d), "
           "2D=(%d,%d), 3D=(%d,%d,%d)\n",
           deviceProp.maxTexture1D, deviceProp.maxTexture2D[0],
           deviceProp.maxTexture2D[1], deviceProp.maxTexture3D[0],
           deviceProp.maxTexture3D[1], deviceProp.maxTexture3D[2]);
    shrLog("  Max Layered Texture Size (dim) x layers        1D=(%d) x %d, "
           "2D=(%d,%d) x %d\n",
...
#if CUDART_VERSION >= 4000
    shrLog("  Concurrent copy and execution:                 %s with %d copy "
           "engine(s)\n",
