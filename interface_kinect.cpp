...
void FreenectDevice_depth_cb(freenect_device* dev, void* v_depth,
                             uint32_t timestamp) {
...
void* FreenectDevice_freenect_threadfunc(void* arg) {
    ...
    while (!dev->stopped()) {
        ...
        if (res < 0 && res != -10) {
            std::cerr << "\nError " << res
                      << " received from libusb - aborting." << std::endl;
            break;
        }
    }
    return NULL;
...
FreenectDevice::FreenectDevice()
    : RGBD(), f_ctx(0L), f_dev(0L), gotDepth(false), die(false) {}
...
int FreenectDevice::open() {
    ...
    int res = pthread_create(&freenect_thread, NULL,
                             FreenectDevice_freenect_threadfunc, this);
...
bool FreenectDevice::available() const { return gotDepth; }

int FreenectDevice::update() { return freenect_process_events(f_ctx); }
